        ...
        <div class="pos_page_inner">
            ...
            <div class=" pos_home_section shop_section shop_fullwidth">
                <div class="row">
                    <div class="col-12">
                        <!--banner slider start-->
                        <div class="slider_active owl-carousel mb-35">
                            @foreach($type_salebanners as $type)
                            <img src="{{asset('frontend/image/sale_banner/'.$type->image)}}" alt="">
                            @endforeach
                        </div>
                        <!--banner slider start-->
                    </div>
                ...
                <div class="shop_tab_product shop_fullwidth">
                    <div class="tab-content" id="myTabContent">
                        <div class="tab-pane fade show active owl-carousel" id="large" role="tabpanel">
                                ...
                                <div class="col-lg-3 col-md-4 col-sm-6" style="margin-bottom: 20px;">
                                    <div class="single_product">
                                        <div class="product_thumb">
                                            <a href="{{route('product_detail',$type->id)}}"><img src="{{asset('frontend/image/product/'.$type->image)}}" width="100%" height="320px;" alt=""></a>
                                            @if($type->quantity==0)
                                            <div class="img_icone">
                                            ...
                                            <a href="{{route('product_detail',$type->id)}}">Chi tiết <i class="fa fa-chevron-circle-right" aria-hidden="true"></i></a>
                                            </div>
                                        </div>
                                        <div class="product_content" style="margin-top: 15px;">
                                            <h3 class="product_title"><a href="{{route('product_detail',$type->id)}}">{{$type->name}}</a></h3>
                                        </div>
                                        <div class="modal_price mb-10" style="margin-top: 10px;">
                                            @if($type->promotion_price==0)
                                            <span class="new_price">{{number_format($type->unit_price,0," ",".")}} ₫</span>
                                            @else
                                            <span class="new_price">{{number_format($type->promotion_price,0," ",".")}} ₫</span>
                                            <span class="old_price">{{number_format($type->unit_price,0," ",".")}} ₫</span>
                                            @endif
                            ...
                            <div>{{$type_products->appends(request()->query())->links('vendor.pagination.bootstrap-4')}}</div>
                            @else
                            <div class="row">
                                <div class="col-12">
                            ...
                            <div class="product_list_item mb-35">
                                <div class="row align-items-center">
                                    <div class="col-lg-3 col-md-5 col-sm-6">
                                        <div class="product_thumb">
                                            <a href="{{route('product_detail',$type->id)}}"><img src="{{asset('frontend/image/product/'.$type->image)}}" width="100%" height="348px;" alt=""></a>
                                            @if($type->quantity==0)
                                            <div class="img_icone">
                                                <span style="position: absolute;background: #ea3a3c;width: 85px;color: #fff;border-radius: 5px;padding: 2px 10px;font-size: 16px;">Hết hàng</span>
                                            </div>
                                    ...
                                    <div class="col-lg-9 col-md-7 col-sm-6">
                                        <div class="list_product_content">
                                            <div class="list_title">
                                                <h3><a href="{{route('product_detail',$type->id)}}">{{$type->name}}</a></h3>
                                            </div>
                                            <div class="product_ratting">
                                                ...
                                                $product_ra = 0;
                                                if ($type->total_ra) {
                                                    $product_ra = round($type->total_number / $type->total_ra, 2);
                                                }
                                                ?>
                                                @for($i=1; $i<=5; $i++) <i class="fa fa-star {{$i <= $product_ra ? 'activestars' : ''}}" style="font-size:16px;color:#999;"></i>
                                                    @endfor
                                                    <a style="margin-left: 10px;color: #ea3a3c;">{{$type->total_ra}} đánh giá</a>
                                                    @endif
                                            </div>
                                            <div class="design">{!!$type->description!!}</div>
