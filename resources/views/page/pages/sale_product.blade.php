    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="slider_active owl-carousel mb-35">
                ...
                <img src="{{asset('frontend/image/sale_banner/'.$sale->image)}}" alt="">
                @endforeach
            </div>
            <!--banner slider start-->
        </div>
    ...
    <div class="shop_tab_product shop_fullwidth">
        <div class="tab-content" id="myTabContent">
            <div class="tab-pane fade show active" id="large" role="tabpanel">
                ...
                <div>{{$promotion_product->appends(request()->query())->links('vendor.pagination.bootstrap-4')}}</div>
                @else
                <div class="row">
                    <div class="col-12">
                ...
                <div class="product_list_item mb-35">
                    <div class="row align-items-center">
                        <div class="col-lg-3 col-md-5 col-sm-6">
                            <div class="product_thumb">
                                <a href="{{route('product_detail',$promotion->id)}}"><img src="{{asset('frontend/image/product/'.$promotion->image)}}" width="100%" height="348px;" alt=""></a>
                                @if($promotion->quantity==0)
                                <div class="img_icone">
                                    <span style="position: absolute;background: #ea3a3c;width: 85px;color: #fff;border-radius: 5px;padding: 2px 10px;font-size: 16px;">Hết hàng</span>
                                </div>
                        ...
                        <div class="col-lg-9 col-md-7 col-sm-6">
                            <div class="list_product_content">
                                <div class="list_title">
                                    <h3><a href="{{route('product_detail',$promotion->id)}}">{{$promotion->name}}</a></h3>
                                </div>
                                <div class="product_ratting">
                                    @if($promotion)
                                    <?php
                                    $product_ra = 0;
                                    if ($promotion->total_ra) {
                                        $product_ra = round($promotion->total_number / $promotion->total_ra, 2);
                                    }
                                    ?>
                                    @for($i=1; $i<=5; $i++) <i class="fa fa-star {{$i <= $product_ra ? 'activestars' : ''}}" style="font-size:16px;color:#999;"></i>
                                        @endfor
                                        <a style="margin-left: 10px;color: #ea3a3c;">{{$promotion->total_ra}} đánh giá</a>
                                        @endif
                                </div>
                                <div class="design">{!!$promotion->description!!}</div>
